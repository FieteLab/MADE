...
class ParameterSpace:
    ...
    def visualize(self, ax:plt.Axes):
        ...
        # if 2D, plot a rectangle
            ...
            w, h = self.ranges[0].end - self.ranges[0].start, self.ranges[1].end - self.ranges[1].start
            rect = plt.Rectangle((self.ranges[0].start, self.ranges[1].start), w, h, fill=True, color='k', alpha=.1)
            ax.add_patch(rect)
...
class Plane(AbstractManifold):
    dim: int = 2
    parameter_space: ParameterSpace = ParameterSpace([Range(0, 1), Range(0, 1)])
